...
public class PessoaController {
    ...
    @GetMapping("/dados-por-estado")
    public ResponseEntity<Map<String, Object>> getDadosPorEstado(@RequestParam("estado") String estado) {
        Map<String, Object> dadosPorEstado = new HashMap<>();

        dadosPorEstado.put("faixaEtariaAndImc", pessoaService.getFaixaEtariaAndImcMedioByEstado(estado));
        dadosPorEstado.put("percentualObesos", pessoaService.getPercentualObesosPorSexoByEstado(estado));
        dadosPorEstado.put("doadoresPorTipoSanguineo", pessoaService.getDoadoresPorTipoSanguineoByEstado(estado));

        return ResponseEntity.ok(dadosPorEstado);
    }
